//! zygfred — a 3-voice FM drum synth for glitch / IDM / Aphex-flavoured percussion.
//!
...
fn setup_midi(tx: Sender<MidiMsg>) -> Option<(MidiInputConnection<()>, String)> {
    let mut input = MidiInput::new("zygfred").ok()?;
    ...
    let conn = input
        .connect(&port, "zygfred-in", move |_s, bytes, _| {
...
fn cc_path() -> PathBuf {
    ...
    PathBuf::from(home).join(".zygfred-midi.txt")
}
...
fn ui(f: &mut RFrame, app: &App) {
    ...
    let outer = Block::default()
        ...
        .title(Span::styled(" \u{25c6} zygfred \u{b7} FM drums ", Style::default().fg(ACCENT).add_modifier(Modifier::BOLD)))
